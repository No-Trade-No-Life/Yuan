...
export default (context: IExtensionContext) => {
  context.registerDeployProvider({
    ...
    make_docker_compose_file: async (ctx, envCtx) => {
      return {
        [`busybox-${ctx.env!.ACCESS_KEY}`.replace(/\s/g, '')]: {
          image: `ghcr.io/no-trade-no-life/tool-busybox:${ctx.version ?? envCtx.version}`,
          restart: 'always',
      ...
      return {
        deployment: {
          ...
          metadata: {
            ...
            name: `busybox-${ctx.key}`.replace(/\s/g, '').toLocaleLowerCase(),
            namespace: 'yuan',
          },
          spec: {
            replicas: 1,
