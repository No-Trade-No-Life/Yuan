# Change Log - @yuants/app-market-data-collector

This log was last generated on Tue, 12 Dec 2023 17:49:32 GMT and should not be manually modified.

## 0.0.45
Tue, 12 Dec 2023 17:49:32 GMT

### Patches

- reset backoff time if succeed

## 0.0.44
Tue, 12 Dec 2023 11:56:04 GMT

### Patches

- fix: task error retry

## 0.0.43
Mon, 11 Dec 2023 13:08:40 GMT

_Version update only_

## 0.0.42
Mon, 11 Dec 2023 11:13:42 GMT

_Version update only_

## 0.0.41
Sun, 10 Dec 2023 15:01:12 GMT

_Version update only_

## 0.0.40
Sat, 09 Dec 2023 20:53:56 GMT

_Version update only_

## 0.0.39
Sat, 09 Dec 2023 19:25:13 GMT

### Patches

- consumeChannel, requestService

## 0.0.38
Sat, 09 Dec 2023 16:37:39 GMT

_Version update only_

## 0.0.37
Sat, 09 Dec 2023 16:08:43 GMT

_Version update only_

## 0.0.36
Sat, 09 Dec 2023 15:43:11 GMT

_Version update only_

## 0.0.35
Sat, 09 Dec 2023 11:57:57 GMT

_Version update only_

## 0.0.34
Fri, 08 Dec 2023 17:01:42 GMT

_Version update only_

## 0.0.33
Fri, 08 Dec 2023 14:47:10 GMT

_Version update only_

## 0.0.32
Fri, 08 Dec 2023 04:56:54 GMT

### Patches

- fix peformance issue for 1k labels

## 0.0.31
Fri, 08 Dec 2023 00:35:02 GMT

### Patches

- refine concepts

## 0.0.30
Thu, 07 Dec 2023 23:18:26 GMT

### Patches

- fix error retry
- fix list watch
- status not set to running

## 0.0.29
Thu, 07 Dec 2023 20:19:54 GMT

### Patches

- refactor using subject pipe pattern

## 0.0.28
Thu, 07 Dec 2023 19:30:49 GMT

_Version update only_

## 0.0.27
Sun, 03 Dec 2023 04:38:50 GMT

_Version update only_

## 0.0.26
Thu, 30 Nov 2023 18:13:09 GMT

### Patches

- retry if query last kline failed

## 0.0.25
Thu, 30 Nov 2023 17:00:34 GMT

### Patches

- enable runOnInit option for retry after crash

## 0.0.24
Fri, 24 Nov 2023 16:07:44 GMT

_Version update only_

## 0.0.23
Fri, 24 Nov 2023 10:40:29 GMT

_Version update only_

## 0.0.22
Tue, 21 Nov 2023 12:39:54 GMT

_Version update only_

## 0.0.21
Tue, 21 Nov 2023 10:59:13 GMT

### Patches

- remove 'MongoDB' hard-coded terminal_id

## 0.0.20
Sat, 18 Nov 2023 18:35:47 GMT

_Version update only_

## 0.0.19
Sat, 18 Nov 2023 17:16:51 GMT

_Version update only_

## 0.0.18
Sat, 18 Nov 2023 15:49:27 GMT

_Version update only_

## 0.0.17
Fri, 17 Nov 2023 18:01:35 GMT

### Patches

- invalid config interrupt the whole program no more

## 0.0.16
Fri, 17 Nov 2023 15:44:25 GMT

### Patches

- add new option for replay periods
- refine json schema

## 0.0.15
Wed, 15 Nov 2023 16:04:13 GMT

### Patches

- polling interval change for storage workload

## 0.0.14
Wed, 08 Nov 2023 19:50:42 GMT

### Patches

- add disable feature

## 0.0.13
Thu, 02 Nov 2023 19:46:45 GMT

_Version update only_

## 0.0.12
Thu, 02 Nov 2023 17:35:35 GMT

_Version update only_

## 0.0.11
Thu, 26 Oct 2023 18:03:30 GMT

_Version update only_

## 0.0.10
Thu, 19 Oct 2023 18:36:28 GMT

_Version update only_

## 0.0.9
Mon, 09 Oct 2023 14:14:56 GMT

### Patches

- fix metrics status

## 0.0.8
Tue, 03 Oct 2023 17:45:31 GMT

### Patches

- use formatTime and UUID from data-model

## 0.0.7
Sat, 23 Sep 2023 10:45:15 GMT

### Patches

- ajust metrics

## 0.0.6
Wed, 06 Sep 2023 15:33:08 GMT

_Version update only_

## 0.0.5
Tue, 05 Sep 2023 17:24:13 GMT

_Version update only_

## 0.0.4
Tue, 05 Sep 2023 10:00:19 GMT

### Patches

- use a new crashloopBackOff logic for job retries
- remove manifests/index.ts
- error printing format

## 0.0.3
Fri, 01 Sep 2023 14:25:44 GMT

_Version update only_

## 0.0.2
Thu, 31 Aug 2023 11:04:40 GMT

### Patches

- support extension

## 0.0.1
Tue, 29 Aug 2023 15:01:01 GMT

### Patches

- init

