...
export default (context: IExtensionContext) => {
  context.registerDeployProvider({
    ...
    make_docker_compose_file: async (ctx, envCtx) => {
      return {
        [`bitget-${ctx.env!.ACCESS_KEY}`.replace(/\s/g, '')]: {
          image: `ghcr.io/no-trade-no-life/vendor-bitget:${ctx.version ?? envCtx.version}`,
          restart: 'always',
        ...
        deployment: {
          ...
          metadata: {
            labels: {
              ...
              'y.ntnl.io/component': 'bitget',
            },
            name: `bitget-${ctx.key}`.replace(/\s/g, '').toLocaleLowerCase(),
            namespace: 'yuan',
          },
          spec: {
            replicas: 1,
