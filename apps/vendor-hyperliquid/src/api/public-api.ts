...
export const getUserFundingHistory = (params: { user: string; startTime?: number; endTime?: number }) =>
  request<{ time: number; hash: string; delta: { type: string; coin: string; usdc: string; szi: string; fundingRate: string } }[]>(
    'POST',
    'info',
...
export const getUserTokenBalances = (params: { user: string }) =>
  request<{ balances: { coin: string; token: number; hold: string; total: string; entryNtl: string }[] }>('POST', 'info', {
    ...params,
    type: 'tokenBalances',
  });

export const getUserOpenOrders = (params: { user: string }) =>
  request<{ coin: string; limitPx: string; oid: number; side: string; sz: string; timestamp: number }[]>('POST', 'info', {
    ...params,
    type: 'openOrders',
  });
