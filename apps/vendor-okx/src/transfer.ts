...
const resOfAssetCurrenciesCache = createCache(
  async (ak: string) => {
    const client = await clientCache.query(ak);
    if (!client) throw new Error('Client not found');
    return client.getAssetCurrencies();
  },
  {
    expire: 3600_000,
  },
...
client$.subscribe(async (client) => {
  ...
  const account_config = await accountConfigCache.query(access_key).then((x) => x!.data[0]);
  console.info(formatTime(Date.now()), 'AccountConfig', JSON.stringify(account_config));
  const { mainUid, uid } = account_config;
      ...
      addAccountTransferAddress({
        ...
        onApply: {
          INIT: async (order) => {
            ...
            const res = await resOfAssetCurrenciesCache.query(access_key);
