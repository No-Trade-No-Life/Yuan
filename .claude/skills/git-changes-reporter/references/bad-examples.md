...
7. [遗漏提交（违反全覆盖原则）](#7-遗漏提交违反全覆盖原则)
8. [提交明细缺失或格式错误](#8-提交明细缺失或格式错误)
9. [提交覆盖检查表格缺失](#9-提交覆盖检查表格缺失)
...
<!-- 报告只覆盖了 2 个提交，但实际范围内有 5 个提交 -->
```

### 为什么错误

- 报告遗漏了部分提交（如版本更新、文档变更等）
- 读者无法确认是否遗漏了重要改动
- 违反全覆盖原则
...
**相关提交**：`c36262cad`, `a1b2c3d4e`

**提交明细**：

- `c36262cad`: 版本号更新至 1.2.0
- `a1b2c3d4e`: 更新 README 文档
...
- 报告中的提交数量必须等于 JSON 中 `commits` 数组的长度
- 每个提交至少出现在一个章节的"相关提交"或"提交明细"中
- 必须包含"提交覆盖检查"表格进行确认
...
- b285cde59 添加了函数 ❌ 缺少冒号分隔
- `76802e0c0` - 优化逻辑 ❌ 使用了横线而非冒号
```

### 为什么错误

- 没有提交明细，读者无法快速了解每个 commit 的具体内容
- 格式不统一，影响可读性和解析
...
- `b285cde59`: 添加 getRequestIntervalMs 函数动态计算 Binance 请求间隔
...
```
- `<commit_hash>`: <一句话描述这个 commit 做了什么>
```
...
- [ ] 所有提交都已在报告中覆盖，无遗漏
- [ ] 每个核心变更章节包含"提交明细"（格式：`hash: 描述`）
- [ ] 报告末尾包含"提交覆盖检查"表格
- [ ] 表格中的提交数量等于 JSON 中 `commits` 数组的长度
- [ ] 每个提交的"所属章节"列填写正确
