...
- [x] 对齐 `VEX_SERIES_DATA_FILTER_NO_INTEREST_RATE`：`apps/virtual-exchange/src/series-data/scheduler.ts` 增加可选 env（默认关闭），`apps/virtual-exchange/src/series-data/DESIGN.md` 同步说明。 | 来源: catch-up：阅读实现+DESIGN.md
- [x] 清理 `apps/virtual-exchange/src/index.ts` 中重复的 `import './series-data'`（之前出现两次）。 | 来源: catch-up：阅读实现
- [x] 对齐 `VEX_SERIES_DATA_FORWARD_SEED_WINDOW_MS` 默认值：`apps/virtual-exchange/src/series-data/DESIGN.md` 改为 1d，与实现一致。 | 来源: catch-up：阅读实现+DESIGN.md
- [x] 补齐“中间缺口（gap）”回补：基于真实数据表（`ohlc_v2`/`interest_rate`）做 gap check（相邻 `created_at` 间隔 `> offsetMs*(1+0.1)`），并引入 `gapQueue` 在 tail-side 优先回补；允许重复写入（依赖唯一键/`ON CONFLICT` 吸收），对 persistent gap 做退避/冷却。 | 来源: 用户反馈：mergeRangesAndGetUnion 的 union 可能掩盖缺口
- [x] 按最新设计简化实现：移除 `gap-check.ts`/gapQueue/真实表 gap check，改为仅基于 `series_data_range` 的 ranges（merge 后）选择 tail time 来回补“距离 now 最近的 gap”；同时简化 VEX 侧限流逻辑（依赖 vendor serviceOptions 兜底）。 | 来源: review R6/R8/R9/R12/R14：设计简化
- [x] 按最新 review 收紧 range merge 语义：merge 只在严格 overlap（`>`）时发生，touch（`=`）不 merge；gap 只在 `prev.end_time < next.start_time` 时成立，避免 touch 触发 tail 回补。 | 来源: plan.md review：strict overlap merge
- [x] 把严格 overlap + 差量 compaction 的 range merge 回填到旧版 `apps/virtual-exchange/src/series-data/scheduler.ts`，并同步修复 tail 的 gap 判定（touch 不算 gap）。 | 来源: 用户指令：用同样 merge 回去改 scheduler 并更新 legion
- [x] 修复 `scheduler.ts` tail 不推进：异常/空 range 时也要 reschedule（带 backoff），并定时打印队列状态便于观察。 | 来源: 用户反馈：head 已空闲但 tailqueue 不带动；需求：定时打印各个 queue 状态
- [x] 每次请求主动制造 overlap：backward/forward 都把 `time` 锚点往已有覆盖区间内部偏移一段 overlap，确保 strict overlap merge 能稳定收敛。 | 来源: 用户指令：每次拉取都要有所重叠

---

_最后更新: 2025-12-25 22:15_
