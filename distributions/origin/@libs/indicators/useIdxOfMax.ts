 ...
 * @param period 周期 (>0)
 */
export const useRollingMaxIndex = (source: Series, period: number) => {
  const iMAX = useSeries(`RollingMaxIndex(${source.name},${period})`, source, {
    // display: "line",
  });
  ...
  useEffect(() => {
    ...
    iMAX[i] = i; // 当前元素
    if (queue.length > 0) {
      if (source[queue[0]] > source[iMAX[i]]) {
        iMAX[i] = queue[0];
      }
    }
  });
