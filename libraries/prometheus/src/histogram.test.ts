...
describe('Histogram', () => {
  test('should create histogram with basic operations', () => {
    ...
    expect(data.buckets.size).toBe(0);
  ...
  test('should delete labeled data', () => {
    ...
    const data = labeledHistogram.get();
    expect(data.count).toBe(0);
    expect(data.sum).toBe(0);
    expect(data.buckets.size).toBe(0);
  });
  ...
  test('should use default buckets if not provided', () => {
    const registry = createRegistry();
    const histogram = registry.histogram('default_histogram', 'Default histogram');
    ...
    const data = histogram.get();
    expect(data.count).toBe(2);
    expect(data.sum).toBe(1.51);

    // 应该使用默认桶
    expect(data.buckets.size).toBeGreaterThan(0);
  });
